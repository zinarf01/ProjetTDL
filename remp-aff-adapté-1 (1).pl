...
#affichage(@automate);
#eps_closure((0),@automate_NFA);
#Move_NFA('a',(0,1,2,4,7),@automate_NFA);
#@automate_DFA=;
determination(0,@automate_NFA);
print"\n NFA \n";
...
sub Move_NFA {
  ...
  my $taille_res_dup = scalar @tab_res_dup ;
   ...
   my $taille_etat_initial=scalar @etat_init_DFA;
   ...
   print("\n ---$taille_etat_initial --- \n");
    for (my $cpt3=0; $cpt3<$taille_etat_initial-$taille_auto_NFA ;$cpt3++){
        print (" \n -$cpt3- $etat_init_DFA[$cpt3] - \n");
   }
   
   my $cpt1=0; 
   $tab_etat_DFA[$cpt1][0]=@etat_init_DFA;
   $tab_etat_DFA[$cpt1][1]='unmarked';
   printf("\n $tab_etat_DFA[$cpt1][0] \n $tab_etat_DFA[$cpt1][1] \n");
   $cpt1++;
   my $taille_etat_DFA= scalar @tab_etat_DFA;
   for (my $i=0; $i<$taille_etat_DFA ;$i++){
        if ($tab_etat_DFA[$i][1] eq 'unmarked'){
        #print ("oh");
        my @T=$tab_etat_DFA[$i][0];
        $tab_etat_DFA[$i][1]='marked';
        
        for (my $j=0;$j<$taille_fct_trans_NFA;$j++){
                if($fct_trans_NFA[$j] ne 'e'){
                    #print("------$fct_trans_NFA[$j]-------");
                 my @S=Move_NFA($fct_trans_NFA[$j],@T,@automate_NFA);
                for (my $k=0;$k<$taille_etat_DFA;$k++){
                    if (@S != $tab_etat_DFA[$k][0]){
                        #print "lolow $k";
